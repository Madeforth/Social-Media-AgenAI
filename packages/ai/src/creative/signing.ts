/**
 * HMAC signing between the Edge Function orchestrator and the Node compositor
 * route. The renderer never receives a provider key or the Supabase
 * service-role key — only a signed request body, so a leaked renderer URL
 * cannot be used to forge arbitrary composites or reach anything privileged.
 */
